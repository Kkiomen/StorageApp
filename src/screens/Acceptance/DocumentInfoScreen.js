...
class DocumentInfoScreen extends Component {
    ...
    constructor({props, navigation}) {
        ...
        this.documentProductsCollection = collection(db, "documentProducts");
        const documentKey = navigation.getParam('documentKey');
        const firestore = getFirestore()
        this.documentDoc = doc(firestore,'documents',documentKey);
        this.state = {
            name: '',
            type: '',
            key: documentKey,
    ...
    async fetchDocument(){
        const documentSnap = await getDoc(this.documentDoc);
        if (documentSnap.exists) {
            const element = documentSnap.data();
            this.setState({
    ...
    async fetchDocumentProduct(){
        const documentKey = this.state.key;
        const arrayProductKey = []
        const q = query(this.documentProductsCollection, where('document', '==', documentKey));
    ...
    saveProduct = () => {
        ...
        choosedProduct['quantity'] = this.state.amountProduct
    ...
    async saveDocument() {
        ...
        updateDoc(doc(firestore,'documents',documentKey), {
            documentNumber: this.state.documentNumber,
        })

        const q = query(collection(db, "documentProducts"), where('document', '==', documentKey));
        let allDocumentProducts = await getDocs(q);

        allDocumentProducts.forEach((res) => {
            if (res.data().document === documentKey) {
                const firestore = getFirestore()
                let docProduct = doc(firestore, 'products', res.data().product);

                if (typeDocument === 'ISSUES') {
                    updateDoc(docProduct, {
                        quantity: parseInt(res.quantity) + parseInt(res.quantity) || 0
                    })
                } else {
                    updateDoc(docProduct, {
                        quantity: parseInt(res.quantity) - parseInt(res.quantity) || 0
                ...
                deleteDoc(doc(firestore, 'documentProducts', res.id));
            }
        });
                        ...
                        this.state.choosedProducts.map((res, i) => {
                            return (
                                <View style={styles.listView} key={i}>
                                    <Grid>
                                        <Col size={3}>
                                            <Text style={[styles.ProductsListTitle, styles.strongText]}>{res.name}</Text>
                                            <Text style={styles.ProductsListAmount}>Quantity: {res.quantity}</Text>
                                            <Text style={[styles.ProductsListTitle, styles.packageInfo]}>Type package: {res.type_package}</Text>
                                        </Col>
                                        <Col size={1}>
                                            <Button
                                                title='X'
                                            ...
                                            return (
                                                <View style={styles.listView}>
                                                    ...
                                                    >
                                                        ...
                                                        <Text style={styles.typePackage}>Type package: {res.type_package}</Text>
                                                    </Pressable>
                                                </View>
                                            );
                                        })
                    ...
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            ...
                            <View style={styles.modalCenter}>
                                ...
                                <Text style={styles.typePackage}>Type package: {this.state.currentProduct.type_package}</Text>
